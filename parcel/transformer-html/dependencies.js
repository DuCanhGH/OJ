    ...
    PostHTML().walk.call(ast.program, (node) => {
        ...
        if (tag === "meta") {
            ...
            if (isMetaDependency) {
                ...
                if (metaAssetUrl) {
                    attrs.content = isDjangoTemplating(attrs.content)
                        ? attrs.content
                        : asset.addURLDependency(attrs.content, {
                              needsStableName: !(
        ...
        if (
            ...
            attrs.href = isDjangoTemplating(href)
                ? href
                : asset.addURLDependency(href, {
        ...
        if (tag === "script" && attrs.src) {
            ...
            if (outputFormat === "esmodule" && !asset.env.supports("esmodules", true)) {
                ...
                attrs.src = asset.addURLDependency(attrs.src, {
                    ...
                    priority: "parallel",
                    bundleBehavior:
                        sourceType === "script" || attrs.async != null ? "isolated" : undefined,
                    env: {
                        sourceType,
                        outputFormat: "global",
                        loc,
            ...
            attrs.src = isDjangoTemplating(attrs.src)
                ? attrs.src
                : asset.addURLDependency(attrs.src, {
                      // Keep in the same bundle group as the HTML.
                      priority: "parallel",
